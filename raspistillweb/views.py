...
TIMELAPSE_DIRECTORY = 'raspistillweb/pictures/'
...
IMAGE_ROTATION_ALERT = 'Please enter a valid Rotation option.'
...
def timelapse_start_view(request):
    global timelapse
    timelapse = True
    filename = strftime("%d-%m-%Y.%H-%M-%S", localtime())
...
def photo_view(request):
    ...
    if timelapse:
        ...
        filename = strftime("%d-%m-%Y.%H-%M-%S.jpg", localtime())
        take_photo(filename)
...
# View for the reboot - no site will be generated
@view_config(route_name='reboot')
def reboot_view(request):
	os.system( 'sleep 10s && sudo reboot' )
	return HTTPFound(location='/')	

# View for the shutdown - no site will be generated
@view_config(route_name='shutdown')
def shutdown_view(request):
	os.system( 'sleep 10s && sudo halt' ) 
	return HTTPFound(location='/')	
...
def take_timelapse(filename):
    ...
    os.chmod(TIMELAPSE_DIRECTORY + filename, 0777)
    call (
        ['raspistill'
        + ' -w ' + str(image_width)
        + ' -h ' + str(image_height)
